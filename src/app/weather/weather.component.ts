...
export class WeatherComponent {
  ...
  // Comprobamos que esta contestado todo el formulario, y miramos si cambiaron los datos o
  // si solo cambiaron las fechas a mostrar. Además guardamos el formulario en el localStorage
  // para que la proxima vez que cargue la pagina, aparezca la última busqueda realizada.
  updateQuestion() {
    if ( this.selectLocation && this.selectMetric && this.question.dateSta && this.question.dateEnd ) {
      if (this.selectLocation !== this.question.location || this.selectMetric !== this.question.metric) {
...
// --------------------------------------------------------------------------------------------------------
  ...
  // Funciones para el datepicker de escoger fecha de inicio.
  chosenYearHandlerSta(normalizedYear: Moment) {
    const ctrlValue = this.datepickerSta.value;
    ctrlValue.year(normalizedYear.year());
    this.datepickerSta.setValue(ctrlValue);
  }
  chosenMonthHandlerSta(normlizedMonth: Moment, datepicker: MatDatepicker<Moment>) {
    const ctrlValue = this.datepickerSta.value;
    ...
    // Funciones para el datepicker de escoger fecha de fin.
    chosenYearHandlerEnd(normalizedYear: Moment) {
      const ctrlValue = this.datepickerEnd.value;
      ctrlValue.year(normalizedYear.year());
      this.datepickerEnd.setValue(ctrlValue);
    }
    chosenMonthHandlerEnd(normlizedMonth: Moment, datepicker: MatDatepicker<Moment>) {
      const ctrlValue = this.datepickerEnd.value;
      ctrlValue.month(normlizedMonth.month());
      this.datepickerEnd.setValue(ctrlValue);
